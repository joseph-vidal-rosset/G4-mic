% =========================================================================
% G4-mic FOL Prover - BETH STRATEGY
% Key principle: γ-rules (eigenvariables) FIRST, δ-rules (instantiation) REUSE ONLY
% Based on Beth tableaux method
...
% prove/8 - WITH LOCAL REGISTRY + BETH STRATEGY
...
% EigenRegistry: list of eigenvariables used in current branch (for γ-rules)
% FreeVars: NOT USED in Beth strategy (kept for compatibility)
...
% =========================================================================
% QUANTIFIER RULES - BETH STRATEGY
% γ-rules (R∀, L∃) MUST come BEFORE δ-rules (L∀, R∃) in clause order!
% =========================================================================

% 8. R∀ - Universal introduction (γ-rule: creates NEW eigenvariable)
...
% 15. L∃ - Existential elimination (γ-rule: creates NEW eigenvariable)
...
% =========================================================================
% δ-rules: BETH STRATEGY - Use ONE witness initially, then ONLY reuse eigenvars
% =========================================================================

% 9. L∀ - Universal elimination (δ-rule: BETH controlled instantiation)
prove(Gamma > Delta, FV, I, J, K, L, Reg, lall(Gamma>Delta, P)) :-
    member((![_Z-X]:A), Gamma),
    % BETH STRATEGY:
    ( Reg = [] ->
        % No eigenvariables yet: use ONE witness constant
        Y = witness_0,
        copy_term((X:A, []), (Y:A1, []))
    ;
        % Eigenvariables exist: ONLY use terms from Reg
        member(EigenTerm, Reg),
        copy_term((X:A, FV), (EigenTerm:A1, FV))
    ),
    prove([A1|Gamma] > Delta, FV, I, J, K, L, Reg, P).

% 16. R∃ - Existential introduction (δ-rule: BETH controlled instantiation)
