%% File: nanocop20.pl  -  Version: 2.0  -  Date: 1 May 2021
%%
...
:- op( 500, fy,all). :- op( 500, fy,ex). :- op(500,xfy,:).nanoCop_decide
...
prove2(F,Set,Proof) :-
    bmatrix(F,Set,Mat), retract_all(lit(_,_,_,_)),
...
prove([Lit|Cla],MI,Path,PI,PathLim,Lem,Set,Proof) :-
    ...
    (-NegLit=Lit;-Lit=NegLit) ->
       ( member(LitL,Lem), Lit==LitL, ClaB1=[], Proof1=[],I=l,V=[]
         ;
